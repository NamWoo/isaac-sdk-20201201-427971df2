...
void FragmentIndex::getSourceIndices(int* indices_ptr, int indices_count) const {
  ISAAC_ASSERT_EQ(indices_count, target_count_);
  for (const auto& fragment : fragments_) {
    std::iota(indices_ptr, indices_ptr + fragment.count, fragment.source_offset);
    indices_ptr += fragment.count;
  }
}
