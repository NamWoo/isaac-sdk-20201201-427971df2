...
class Behavior : public alice::Codelet {
 public:
  ...
  // Starts the child nodes at the indices identified by `indices` as a batch. The status of all
  // children is reset to `RUNNING` in the node backend before they are started sequentially to
  // avoid codelets performing actions based on an uninitialized child node status.
  void startChildren(const std::vector<int>& indices);
