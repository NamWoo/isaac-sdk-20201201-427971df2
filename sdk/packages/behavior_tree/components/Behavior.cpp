...
void Behavior::startChildren(const std::vector<int>& indices) {
  std::vector<alice::Node*> nodes;
  for (const int index : indices) {
    nodes.push_back(&children().getNodeByIndex(index));
  }
  node()->app()->backend()->node_backend()->startNodes(nodes);
}
