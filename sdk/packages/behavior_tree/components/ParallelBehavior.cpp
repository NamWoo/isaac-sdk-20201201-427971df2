...
void ParallelBehavior::start() {
  ...
  // Start all children
  std::vector<int> child_indices_to_start(getNumChildren());
  std::iota(child_indices_to_start.begin(), child_indices_to_start.end(), 0);
  startChildren(child_indices_to_start);
}
