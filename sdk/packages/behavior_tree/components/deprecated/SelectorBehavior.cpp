...
void SelectorBehavior::tick() {
  ...
  if (desired == current_behavior_) {
    if (get_report_child_status()) {
      const auto status = behavior_->get(current_behavior_)->getStatus();
      if (status == alice::Status::SUCCESS) {
        reportSuccess("Child behavior reported success");
      } else if (status == alice::Status::FAILURE) {
        reportFailure("Child behavior reported failure");
      }
    }
    return;
  }
