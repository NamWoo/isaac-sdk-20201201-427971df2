...
// RealsenseCamera is an Isaac codelet for the Realsense D435 camera, which can provide color,
// depth, and infrared (IR) images.
//
...
// are 90, 60, 30, 15, 6 FPS. The camera can also produce images at a 1080p resolution. However,
// this is currently not supported as color and depth are set to the same resolution.
class RealsenseCamera : public alice::Codelet {
 public:
  ...
  // The left IR camera image
  ISAAC_PROTO_TX(ImageProto, left_ir);
  // The right IR camera image
  ...
  // Intrinsics including pinhole and distortion parameters for the color camera
  ISAAC_PROTO_TX(CameraIntrinsicsProto, color_intrinsics);
  // Intrinsics including pinhole and distortion parameters for the left IR camera
  ISAAC_PROTO_TX(CameraIntrinsicsProto, left_ir_intrinsics);
  // Intrinsics including pinhole and distortion parameters for the right IR camera
  ...
  // It should match the depth map framerate due to the RS 435 firmware constraints.
  ...
  // It should match the IR framerate due to the RS 435 firmware constraints.
