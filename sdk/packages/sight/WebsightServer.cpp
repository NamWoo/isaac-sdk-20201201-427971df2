...
void WebsightServer::start() {
  tickBlocking();
  redis_backend_ = node()->app()->backend()->redis_backend();
  ...
  hub_->onConnection([this](uWS::WebSocket<uWS::SERVER> *ws, uWS::HttpRequest req) {
    ...
    redis_backend_->command("HSET isaac:%s:webserver:%lld connect %lld",
                            node()->app()->uuid().c_str(), reinterpret_cast<int64_t>(ws),
  ...
  hub_->onDisconnection([this](uWS::WebSocket<uWS::SERVER> *ws,  int code, char *message,
    ...
    redis_backend_->command("HSET isaac:%s:webserver:%lld disconnect %lld",
                            node()->app()->uuid().c_str(), reinterpret_cast<int64_t>(ws),
                            node()->clock()->timestamp());
  ...
  hub_->onMessage([this](uWS::WebSocket<uWS::SERVER>* ws, char* message, size_t length,
    ...
    if (message_type == "getAllTags") {
      ...
      json_out["type"] = "pong";
      json_out["data"] = *maybe_data;
      const int64_t bandwidth = socket_servers_[ws].flow_control.bandwidth(ToSeconds(NowCount()));
      json_out["data"]["bandwidth"] = bandwidth;

      redis_backend_->command("HSET isaac:%s:webserver:%lld bandwidth %lld",
                              node()->app()->uuid().c_str(), reinterpret_cast<int64_t>(ws),
                              bandwidth);

      auto latency_ms = maybe_data->find("latency_ms");
      if (latency_ms != maybe_data->end()) {
        redis_backend_->command("HSET isaac:%s:webserver:%lld latency_ms %lld",
                                node()->app()->uuid().c_str(), reinterpret_cast<int64_t>(ws),
                                latency_ms->get<int64_t>());
      }
      // Reset the bandwidth
      // TODO(ben): This should be done only when bandwidth is changing. Once we have callback on
      // config change we can do it properly.
      socket_servers_[ws].flow_control.resetTargetBandwith(get_bandwidth());
...
Json WebsightServer::getLatestMessages(uWS::WebSocket<uWS::SERVER>* ws) {
  ...
  Json json_data;
  for (const auto& kvp : latest_) {
    addTags(kvp.first);
    auto json = kvp.second->json;
    json["uuid"] = channel_uuid_[kvp.first];
    json["name"] = kvp.first;
...
void WebsightServer::tick() {
  {
    ...
    message_ledger_->iterateAllQueues(
        [this, use_compression](const alice::MessageLedger::Endpoint& endpoint,
                                alice::ConstMessageQueue& messages) {
      // Get latest messages
      messages.sync();

      // Store latest sight message from outside channels for getLatestMessages
      if (endpoint.component != this && !messages.empty()) {
        if (const auto ptr =
            std::dynamic_pointer_cast<const alice::SightMessage>(messages.latest())) {
          latest_[endpoint.nameWithApp()] = std::move(ptr);
        }
      }

      // Process all messages from all endpoints
      while (!messages.empty()) {
        const auto* message = messages.peek().get();
        // Currently we can only handle JsonMessage and SightMessage
        if (const auto* sight_message = dynamic_cast<const alice::SightMessage*>(message)) {
          processNewSightMessage(endpoint, sight_message, use_compression);
        } else if (const auto* json_message = dynamic_cast<const alice::JsonMessage*>(message)) {
          processNewJsonMessage(endpoint, json_message);
        } else {
          LOG_ERROR("Unknown message type. Can only handle JsonMessage and SightMessage.");
        }
        // Pop message after it was sent
        messages.pop();
      }
    });
  }
  hub_->getLoop()->doEpoll(get_latency());
}
...
const std::vector<char>& WebsightServer::loadCompressedResource(const std::string& filename,
  ...
  const bool use_cache = get_use_cache();
  if (use_cache) {
  ...
  if (!use_cache) {
    auto it = compressed_resources_.find(filename);
    if (it != compressed_resources_.end()) {
      it->second = std::move(buffer_compressed);
      return it->second;
    }
  }
