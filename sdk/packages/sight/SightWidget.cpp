...
std::optional<std::pair<std::string, Json>> SightWidget::toJson() const {
  ...
  // Add pose markers to show in widget
  Json markers_json;
  for (const auto& marker : get_markers()) {
    Json marker_json;
    marker_json["name"] = marker;
    markers_json.push_back(marker_json);
  }
  if (!markers_json.empty()) {
    json["markers"] = std::move(markers_json);
  }
