...
function sightDrawHandle(ops) {
  RendererManager().process(ops);
};
...
function CreateNewWindow(type, show_config = true, default_name = '') {
  ...
  // Handle empty name or abort.
  if (!name) return null;
  // Make sure no renderer with the same name exist, otherwise output an alert to let the user know.
  if (renderer_manager_.renderers_.hasOwnProperty(name)) {
    alert("A renderer with the name `" + name + "` already exists.");
    return null;
  }
  // Create the requested renderer.
  let renderer = null;
