...
class Renderer3D extends SightRenderer {
  ...
  updateInactiveMarkers() {
    try {
      let updatedMarkers = {};
      for (let i in this.markers_channels_) {
        let marker = this.markers_channels_[i];
        let path = marker.c;
        // let marker = this.markers_[markerPath];
        // let pathEndNode = marker.rhs;
        if (!PoseTree().areConnected(this.base_referential_frame_, path)) continue;
        let poseMat = PoseTree().get(this.base_referential_frame_, path);
        // Decompose matrix in rotation and translation
        let tmpTrans = new THREE.Vector3();
        let tmpQuat = new THREE.Quaternion();
        let tmpScale = new THREE.Vector3();
