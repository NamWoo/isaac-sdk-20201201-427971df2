...
class MapContainerImpl {
  constructor() {
    ...
    this.editing_action_callback_ = null;
    const MapEvents = {
  ...
  /* Sets the callback for when the edition action change */
  setEditingActionCallback(callback) {
    this.editing_action_callback_ = callback;
  }
  ...
  /* Returns whether we are currently in a drawing mode */
  isDrawingAction() {
    return this.current_editing_action_ == this.EditingAction.DRAWING_WAYPOINTS ||
           this.current_editing_action_ == this.EditingAction.DRAWING_POLYGONS;
  }
  ...
  */
  ...
  /* Update status of all layers when layer radio boxes are programmatically selected. */
  ...
  getEditingTools_(layer_type, is_drawing) {
    ...
    if (this.editing_action_callback_) {
      this.editing_action_callback_();
    }
