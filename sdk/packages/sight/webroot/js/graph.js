...
function UpdateAppGraph(message, div) {
  // If the div is not visible, we can delay any changes:
  if (!div.isVisible()) {
    if (!div.message) {
      // Let's try again in 1s
      setTimeout(function() {
        if (div.message) {
          UpdateAppGraph(div.message, div);
        }
      }, 1000);
    }
    div.message = message;
...
// Add a search bar to highlight some nodes.
function AddSearchBar(div) {
  let search_div = document.createElement("div");
  search_div.classList.add("tab");
  search_div.classList.add("tab_graph");
  div.parentNode.appendChild(search_div);
  div.input_elem = document.createElement("input");
  div.input_elem.type = "text";
  div.input_elem.placeholder = "Filter";
  search_div.appendChild(div.input_elem);

  // Only display the codelet of a selected node.
  div.input_elem.highlighGraph = function(evt) {
    // If escape is pressed, erase the input
    if (evt && evt.keyCode == 27) {
      div.input_elem.value = "";
    }
    div.filter_str = div.input_elem.value;
    let bmatch = div.filter_str.length;
    // Compute the best match
    let nodes = div.cy.nodes();
    for (let i = 0; i < nodes.length; i++) {
      bmatch = Math.min(bmatch, TextFilter(nodes[i].data().fullname, div.filter_str,
                                           div.filter_str.length + 10));
    }
    for (let i = 0; i < nodes.length; i++) {
      let match = TextFilter(nodes[i].data().fullname, div.filter_str,
                             div.filter_str.length + 10)
      if (match > bmatch + 1) {
        nodes[i].addClass("unselected-node");
      } else {
        nodes[i].removeClass("unselected-node");
      }
    }
  };
  div.input_elem.onkeyup = div.input_elem.highlighGraph;
}
...
function IsInterfaceEndpoint(endpoint) {
  return endpoint.indexOf("/Subgraph/") != -1 || endpoint.indexOf("/interface/") != -1;
}
