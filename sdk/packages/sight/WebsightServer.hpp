...
class WebsightServer : public isaac::alice::Codelet {
 public:
  ...
  // Change the latency sight messages are processed. Higher latency can help reduce the stress on
  // the connection, however the message will be delayed before being sent.
  ISAAC_PARAM(int, latency, 5);
  // Whether to compress data for transfer
  ISAAC_PARAM(bool, use_compression, true);
  // Whether to cache the file we are loading from disk. If set to false then Websight server will
  // read from disk everytime a request is made.
  ISAAC_PARAM(bool, use_cache, true);
 ...
 private:
  ...
  alice::RedisBackend* redis_backend_;

  std::map<std::string, std::shared_ptr<const alice::SightMessage>> latest_;
};
