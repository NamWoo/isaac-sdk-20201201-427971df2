...
// All derived class needs to do is to define protoToRos() method.
...
// Set Latch=true to make the last published message available to future subscribers.
...
class ProtoToRosConverter : public alice::Codelet {
 public:
  ...
  void tick() override {
    ...
    if (!protoToRos(rx_proto(), ros_time, ros_message)) {
      return;
    }
    // Publish ROS message
  ...
  virtual bool protoToRos(const alice::ProtoRx<IsaacProtoType>& rx_proto,
                          const ros::Time& ros_time, RosMsgType& ros_message) = 0;
 ...
 private:
  // Message hook for the incoming Isaac data, which will be published to ROS after conversion.
  // Manually define rx_proto_ variable and rx_proto() function instead of using ISAAC_PROTO_RX
  // macro to disallow derived converters from accessing them directly.
  const alice::ProtoRx<IsaacProtoType>& rx_proto() const { return rx_proto_; }
  alice::ProtoRx<IsaacProtoType> rx_proto_{this, "proto"};
