...
// All derived class needs to do is to define rosToProto() method.
...
class RosToProtoConverter : public alice::Codelet {
 public:
  ...
  void convertAndPublish(const typename RosMsgType::ConstPtr& ros_message) {
    ...
    if (!rosToProto(ros_message, ros_time, tx_proto())) {
      return;
    }
    // Publish
    if (ros_time) {
  ...
  virtual bool rosToProto(const typename RosMsgType::ConstPtr& ros_message,
                          ...
                          alice::ProtoTx<IsaacProtoType>& tx_proto) = 0;
 ...
 private:
  // Message hook for the outgoing Isaac data, which will be published after receiving and
  // converting ROS message.
  // Manually define tx_proto_ variable and tx_proto() function instead of using ISAAC_PROTO_TX
