...
// a ImageProto message containing the RGB image and a CameraIntrinsicsProto message containing
...
class ImageLoader : public alice::Codelet {
 ...
 private:
  ...
  // Output: Publishes the image
  void createAndPublishColorImageMessage(Image3ub image, const int64_t acqtime);
  // Input: Dimensions of the image to be published and the image's acquired time
  // Output: Sets pinhole and distortion and publishes the intrinsics message
  // Assumes .png images don't contain alpha channel, only rgb
  void createAndPublishCameraIntrinsicsMessage(const int rows, const int cols,
