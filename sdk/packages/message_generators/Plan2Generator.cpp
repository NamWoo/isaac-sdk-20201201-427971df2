...
// Tolerance to use in the outgoing goal message
...
void Plan2Generator::tick() {
  ...
  // Publish the last pose of the plan
  auto goal_proto = tx_goal().initProto();
  if (maybe_waypoints->empty()) {
    goal_proto.setStopRobot(true);
  } else {
    ToProto(maybe_waypoints->back(), goal_proto.initGoal());
  }
  goal_proto.setTolerance(kGoalTolerance);
  goal_proto.setGoalFrame(plan_frame);
  tx_goal().publish();
  ...
  if (get_report_success()) {
    reportSuccess("Successfully published plan");
  }
}

void Plan2Generator::stop() {
  if (get_erase_path_on_stop()) {
    show("path", [&](sight::Sop& sop) {});
  }
}
