...
void CameraGenerator::tick() {
  ...
  // left color image
  ToProto(std::move(buffer_color_left), tx_color_left().initProto(),
          tx_color_left().buffers());
  tx_color_left().publish(acqtime);
  // right color image
  ToProto(std::move(buffer_color_right), tx_color_right().initProto(),
          tx_color_right().buffers());
  tx_color_right().publish(acqtime);
  // depth image
  ToProto(std::move(buffer_depth), tx_depth().initProto(), tx_depth().buffers());
