...
def create_dolly_docking_pipeline(app, config):
    ...
    for i in range(num_agents):
        ...
        app.connect(tcp_sub_array[sim_index],
                    'base_state_{}'.format(agent_index + 1), codelet, 'state')
    ...
    for i in range(num_agents):
        ...
        codelet = pid.add(app.registry.isaac.controller.DifferentialBaseControl)
        codelet.config['tick_period'] = '100Hz'
        codelet.config['robot_frame'] = 'robot_{}'.format(i + 1)
        ...
        app.connect(codelet, 'cmd', tcp_pub_array[sim_index],
                    'base_command_{}'.format(agent_index + 1))
    ...
    for i in range(num_agents):
        ...
        app.connect(reset_codelet, 'sim{}'.format(i + 1), tcp_pub_array[sim_index],
                    'teleport_{}'.format(agent_index + 1))
    ...
    for i in range(num_agents):
        ...
        sim_index, agent_index = get_sim_and_agent_index(i, config)
        app.connect(tcp_sub_array[sim_index],
                    'rangescan_{}'.format(agent_index + 1), codelet, 'scan')
    ...
    for i in range(num_agents):
        ...
        sim_index, agent_index = get_sim_and_agent_index(i, config)
        app.connect(tcp_sub_array[sim_index],
                    'bodies_{}'.format(agent_index + 1), codelet, 'scene_pose')
        app.connect(tcp_sub_array[sim_index], 'collision_{}'.format(agent_index + 1), codelet,
                    'body_collision')
...
def py_reinforcement_learning_app(config):
    ...
    app = Application(
        ...
        modules = [
            'engine_tcp_udp', 'ml', 'navigation', 'planner', 'perception',
...
def frozen_model_inference_app(config):
    ...
    app = Application(
        name = 'dolly_navigation',
        modules = [
            'controller', 'engine_tcp_udp', 'ml', '//packages/ml:tensorflow', 'navigation',
            'planner', 'perception', '//packages/rl/components:rl',
            '//packages/rl/apps/dolly_navigation/components:dolly_navigation_task',
            'sight', 'utils',
        ])
    # Add the codelets to the reinforcement learning pipeline along with relevant
