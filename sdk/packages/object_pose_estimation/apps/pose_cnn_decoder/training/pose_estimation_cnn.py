...
class PoseEstimationCNN(object):
    ...
    """
    def __init__(self, encoder, encoder_bbox, regression_translation, regression_rotation,
    ...
    def loss(self):
        ...
        if self._norm_regularize > 0:
            decoder_loss += self._encoder.reg_loss * tf.constant(self._norm_regularize,
                                                                 dtype=tf.float32)
            decoder_loss += self._encoder_bbox.reg_loss * tf.constant(self._norm_regularize,
                                                                      dtype=tf.float32)
