...
class RegressionRotation(object):
    ...
    def rotation_prediction(self):
        ...
        for num_fc_layer_units in self._num_fc_layers_rotation:
            ...
            tf.compat.v1.summary.histogram("rotation layer1", rotation_prediction)
    ...
    def quaternion_acos_loss(self, i_symm):
        ...
        quat_sum_squares = tf.math.reduce_sum(tf.math.square(self._rotation_prediction), axis=1)
        ...
        quaternions_dot = tf.math.reduce_sum(tf.multiply(self._input_rotation[:, i_symm],
                                                         self._rotation_prediction),
                                             axis=1)

        quaternions_dot = tf.math.divide(tf.math.scalar_mul(2.0, tf.math.square(quaternions_dot)),
                                         quat_sum_squares)
        ...
        cos_value_clip = tf.clip_by_value(cos_rotation_value,
                                          clip_value_min=-1.0 + 1e-5,
                                          clip_value_max=1.0 - 1e-5)
        # Output must be batch_size x 1 after this
        quat_loss_batch = tf.math.abs(
            tf.math.acos(cos_value_clip, name="quat_arc_coss_batch_loss"))
    ...
    def points_rotation_loss(self, i_symm):
        ...
        body_points = tf.expand_dims(
            tf.constant([[0.5, -0.5, 0.5, -0.5, 0.5, -0.5, 0.5, -0.5],
        ...
        points_rotation_loss_square = tf.reduce_sum(tf.square(points_rotation_matrix_loss),
                                                    axis=1)
    ...
    def rotation_loss(self):
        ...
        quaternion_magnitude_loss = tf.compat.v1.losses.absolute_difference(
            tf.ones_like(quat_sum_squares), quat_sum_squares)
