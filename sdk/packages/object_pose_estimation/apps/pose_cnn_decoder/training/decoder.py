...
class Decoder(object):
    ...
    def x(self):
        ...
        layer_dimensions = [[
            int(h / np.prod(self._strides[i:])),
            int(w / np.prod(self._strides[i:]))
        ] for i in range(len(self._strides))]
        x = tf.layers.dense(inputs=self._latent_code,
                            units=layer_dimensions[0][0] * layer_dimensions[0][1] *
                            self._num_filters[0],
        ...
        x = tf.reshape(x,
                       [-1, layer_dimensions[0][0], layer_dimensions[0][1], self._num_filters[0]])
    ...
    def reconstr_loss(self):
        ...
        """
        tf.compat.v1.summary.image('decoder_output', self.x, max_outputs=1)
        ...
        if self._loss == 'L2':
            if self._bootstrap_ratio > 1:
                l2 = tf.compat.v1.losses.mean_squared_error(
            ...
            if self._bootstrap_ratio > 1:
                l1 = tf.losses.absolute_difference(reconstruction_target_flat,
                                                   xmask_flat,
                                                   reduction=tf.compat.v1.losses.Reduction.NONE)
        ...
        tf.compat.v1.summary.scalar('reconstruction_loss', loss)
        if self._auxiliary_mask:
            mask_loss = tf.compat.v1.losses.mean_squared_error(
                tf.cast(
                    tf.greater(tf.reduce_sum(self._reconstruction_target, axis=3, keepdims=True),
                               0.0001), tf.float32),
                self._xmask,
                reduction=tf.compat.v1.losses.Reduction.MEAN)
            loss += mask_loss
