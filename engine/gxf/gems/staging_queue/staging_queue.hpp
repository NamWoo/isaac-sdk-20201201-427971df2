...
#include "gxf/gems/staging_queue/staging_queue_iterator.hpp"
...
template <typename T>
void StagingQueue<T>::push(T item) {
  std::lock_guard<std::mutex> lock(mutex_);
  const size_t begin_backstage = begin_ + num_mainstage_;
  if (num_backstage_ == capacity_) {
    ...
    switch (overflow_behavior_) {
      ...
      case OverflowBehavior::kFault:
        PANIC("Added an item to a full queue while using the 'Fault' overflow behavior.");
      default:
  ...
  if (num_mainstage_ > capacity_) {
    switch (overflow_behavior_) {
      ...
      case OverflowBehavior::kReject:
        ...
        break;
      case OverflowBehavior::kFault:
        PANIC("Added items to a full queue while using the 'Fault' overflow behavior.");
        break;
      default:
        PANIC("Invalid parameter");
    }
