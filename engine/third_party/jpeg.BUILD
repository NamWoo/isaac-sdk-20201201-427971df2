...
load("@com_nvidia_isaac_engine//engine/build:template.bzl", "template_rule")

libjpegturbo_nocopts = ["-Wno-error"]
...
cc_library(
    ...
    copts = libjpegturbo_copts + libjpegturbo_nocopts,
    includes = ["."],
    visibility = ["//visibility:public"],
    deps = select({
        "@com_nvidia_isaac_engine//engine/build:cpu_aarch64": [":simd_aarch64"],
        "@com_nvidia_isaac_engine//engine/build:cpu_host": [":simd_x86_64"],
        #"//conditions:default": [":simd_x86_64"],
    }),
)
...
# FIXME SIMD version for aarch64 is not compilng
cc_library(
    name = "simd_aarch64",
    srcs = [
        "simd/jsimd_arm64.c",
...
JCONFIG_COMMON_SUBSTITUTIONS = {
    "#define JPEG_LIB_VERSION  62": "#define JPEG_LIB_VERSION  80",
...
genrule(
    ...
    outs = ["jconfig.h"],
    cmd = select({
        "@com_nvidia_isaac_engine//engine/build:platform_jetpack44": "cp $(location jconfig_nowin_simd.h) $@",
        "@com_nvidia_isaac_engine//engine/build:platform_x86_64": "cp $(location jconfig_nowin_simd.h) $@",
    }),
)
...
genrule(
    ...
    outs = ["jconfigint.h"],
    cmd = "cp $(location jconfigint_nowin.h) $@",
)
